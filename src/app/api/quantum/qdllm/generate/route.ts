...
export async function POST(request: NextRequest) {
  try {
    ...
    const pythonScript = path.join(process.cwd(), 'src/lib/quantum/qdllm_interface.py')
    const options = {
      mode: 'text' as const,
      pythonPath: 'python',
      pythonOptions: ['-u'],
    ...
    console.log(`qdLLM generation - User: ${user.id}, Method: ${resultData.method}, Time: ${resultData.processingTime}s`)
    ...
    console.error('qdLLM generation error:', error)
